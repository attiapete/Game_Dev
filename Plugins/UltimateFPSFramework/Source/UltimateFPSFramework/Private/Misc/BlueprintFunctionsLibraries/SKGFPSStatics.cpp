...
void USKGFPSStatics::SpawnEffects(const FHitResult& HitResult, const FSKGImpactEffects& ImpactEffects, bool bDelayed)
{
	if (const AActor* WorldActor = HitResult.GetActor())
	{
		if (const UWorld* World = WorldActor->GetWorld())
		{
			...
			const float Delay = GetSpeedOfSoundDelayActor(HitResult.GetActor(), UGameplayStatics::GetPlayerPawn(World, 0));
...
void USKGFPSStatics::PlaySoundDelayed(const AActor* WorldActor, const FVector& Location, FSKGSoundSettings SoundSettings)
{
	...
	{
		if (const APawn* LocalPawn = UGameplayStatics::GetPlayerPawn(WorldActor->GetWorld(), 0))
		{
			const float Delay = GetSpeedOfSoundDelayLocation(Location, LocalPawn->GetActorLocation());
			if (Delay < 0.15f)
			{
...
TArray<ASKGLightLaser*> USKGFPSStatics::GetLightsLasers(ASKGFirearm* Firearm)
{
	...
	if (IsValid(Firearm))
	{
		TArray<USKGAttachmentComponent*> PartComponents = ISKGFirearmPartsInterface::Execute_GetCachedComponents(Firearm).LightLasers;
...
TArray<ASKGMagnifier*> USKGFPSStatics::GetMagnifiers(ASKGFirearm* Firearm)
{
	...
	if (IsValid(Firearm))
	{
		TArray<USKGAttachmentComponent*> PartComponents = ISKGFirearmPartsInterface::Execute_GetCachedComponents(Firearm).Magnifiers;
...
TArray<ASKGPart*> USKGFPSStatics::GetRenderTargets(ASKGFirearm* Firearm)
{
	...
	if (IsValid(Firearm))
	{
		TArray<USKGAttachmentComponent*> PartComponents = ISKGFirearmPartsInterface::Execute_GetCachedComponents(Firearm).RenderTargets;
...
ASKGBarrel* USKGFPSStatics::GetBarrel(ASKGFirearm* Firearm)
{
	if (IsValid(Firearm))
	{
		const USKGAttachmentComponent* PartComponent = ISKGFirearmPartsInterface::Execute_GetCachedComponents(Firearm).Barrel;
...
ASKGHandguard* USKGFPSStatics::GetHandguard(ASKGFirearm* Firearm)
{
	if (IsValid(Firearm))
	{
		const USKGAttachmentComponent* PartComponent = ISKGFirearmPartsInterface::Execute_GetCachedComponents(Firearm).Handguard;
		if (IsValid(PartComponent))
		{
...
ASKGPart* USKGFPSStatics::GetStock(ASKGFirearm* Firearm)
{
	if (IsValid(Firearm))
	{
		const USKGAttachmentComponent* PartComponent = ISKGFirearmPartsInterface::Execute_GetCachedComponents(Firearm).Stock;
		if (IsValid(PartComponent))
		{
...
ASKGForwardGrip* USKGFPSStatics::GetForwardGrip(ASKGFirearm* Firearm)
{
	if (IsValid(Firearm))
	{
		const USKGAttachmentComponent* PartComponent = ISKGFirearmPartsInterface::Execute_GetCachedComponents(Firearm).ForwardGrip;
		if (IsValid(PartComponent))
		{
...
ASKGMuzzle* USKGFPSStatics::GetMuzzle(ASKGFirearm* Firearm)
{
	if (IsValid(Firearm))
	{
		const USKGAttachmentComponent* PartComponent = ISKGFirearmPartsInterface::Execute_GetCachedComponents(Firearm).Muzzle;
		if (IsValid(PartComponent))
		{
