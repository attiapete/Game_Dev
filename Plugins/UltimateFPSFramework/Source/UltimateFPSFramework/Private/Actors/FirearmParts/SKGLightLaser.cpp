...
void ASKGLightLaser::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_WITH_PARAMS_FAST(ASKGLightLaser, LightLaserState, Params);
	DOREPLIFETIME_WITH_PARAMS_FAST(ASKGLightLaser, LightLaserSettings, Params);
	DOREPLIFETIME_WITH_PARAMS_FAST(ASKGLightLaser, bInfraredMode, Params);
}
...
void ASKGLightLaser::BeginPlay()
{
	...
	if (LaserMesh && !LaserMesh->GetStaticMesh())
	{
		LaserMesh->DestroyComponent();
		LaserDecalComponent->DestroyComponent();
	}
...
{
	return CharacterComponent ? CharacterComponent.Get()->NightVisionOn() : false;
}
...
void ASKGLightLaser::SetLightLaserState_Implementation(const ESKGLightLaserState State)
{
	if (LightLaserState != State)
	{
		...
		{
			MARK_PROPERTY_DIRTY_FROM_NAME(ASKGLightLaser, LightLaserState, this);
			ForceNetUpdate();
		}
	}
}
...
void ASKGLightLaser::Server_SetLaserColorIndex_Implementation(uint8 Index)
{
	...
	MARK_PROPERTY_DIRTY_FROM_NAME(ASKGLightLaser, LightLaserSettings, this);
...
void ASKGLightLaser::Server_SetInfraredMode_Implementation(bool bInfrared)
{
	...
	MARK_PROPERTY_DIRTY_FROM_NAME(ASKGLightLaser, bInfraredMode, this);
	ForceNetUpdate();
	OnRep_InfraredMode();
}
