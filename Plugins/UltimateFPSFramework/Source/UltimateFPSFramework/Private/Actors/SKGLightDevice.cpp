...
{
	if (bInfraredMode && (CharacterComponent && !CharacterComponent->NightVisionOn()))
	{
		bOn = false;
	}
...
void ASKGLightDevice::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_WITH_PARAMS_FAST(ASKGLightDevice, LightMode, Params);
	DOREPLIFETIME_WITH_PARAMS_FAST(ASKGLightDevice, bInfraredMode, Params);
}
...
void ASKGLightDevice::Server_SetInfraredMode_Implementation(bool bInfrared)
{
	if (LightDeviceSettings.bSupportsInfrared)
	{
		...
		MARK_PROPERTY_DIRTY_FROM_NAME(ASKGLightDevice, bInfraredMode, this);
		OnRep_InfraredMode();
	}
}
...
{ // CHECK IF EXISTS IN LightModes
	LightMode = Mode;
	MARK_PROPERTY_DIRTY_FROM_NAME(ASKGLightDevice, LightMode, this);
	OnRep_LightMode();
}
