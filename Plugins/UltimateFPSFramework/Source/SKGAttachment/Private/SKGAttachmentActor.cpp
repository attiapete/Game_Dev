...
void ASKGAttachmentActor::PostInitializeComponents()
{
	...
	if (HasAuthority())
	{
		...
		MARK_PROPERTY_DIRTY_FROM_NAME(ASKGAttachmentActor, AttachmentComponents, this);
	}
	if (const USKGAttachmentDeveloperSettings* Settings = GetDefault<USKGAttachmentDeveloperSettings>())
	{
		TArray<UActorComponent*> ShapeComponents = GetComponentsByTag(UShapeComponent::StaticClass(), Settings->AttachmentOverlapTag);
	...
	{
		const FString Message = FString::Printf(TEXT("%s: AttachmentMesh Does NOT have the ComponentTag set in the project settings added, Set this"), *GetName());
...
void ASKGAttachmentActor::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_WITH_PARAMS_FAST(ASKGAttachmentActor, AttachmentComponents, Params);
	DOREPLIFETIME_WITH_PARAMS_FAST(ASKGAttachmentActor, CurrentOffset, Params);

	FDoRepLifetimeParams ParamsCondition;
	ParamsCondition.bIsPushBased = true;
	DOREPLIFETIME_WITH_PARAMS_FAST(ASKGAttachmentActor, OwningAttachmentComponent, ParamsCondition);
}
...
void ASKGAttachmentActor::SetOwningAttachmentComponent_Implementation(USKGAttachmentComponent* AttachmentComponent)
{
	OwningAttachmentComponent = AttachmentComponent;
	if (HasAuthority())
	{
		MARK_PROPERTY_DIRTY_FROM_NAME(ASKGAttachmentActor, OwningAttachmentComponent, this);
		ForceNetUpdate();
	}
}
...
{
	CurrentOffset = Offset;
	MARK_PROPERTY_DIRTY_FROM_NAME(ASKGAttachmentActor, CurrentOffset, this);
	ForceNetUpdate();
	OnRep_CurrentOffset();
}
