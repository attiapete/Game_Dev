...
int32 USKGAttachmentFunctionLibrary::MaxAttachmentStack = 10;
...
USKGAttachmentManager* USKGAttachmentFunctionLibrary::GetOwningAttachmentManager(AActor* Actor)
{
	...
	if (IsValid(Actor))
	{
		...
		else
		{
			...
			for (uint8 i = 0; i < MaxAttachmentStack; ++i)
			{
...
UMeshComponent* USKGAttachmentFunctionLibrary::SetupAttachmentMesh(AActor* Actor)
{
	if (IsValid(Actor))
	{
		...
		if (const USKGAttachmentDeveloperSettings* Settings = GetDefault<USKGAttachmentDeveloperSettings>())
		{
			...
			{
				if (IsValid(MeshComponent) && MeshComponent->ComponentHasTag(Settings->AttachmentMeshTag))
				{
					bFoundWithTag = true;
...
#if WITH_EDITOR
		const FString Message = FString::Printf(TEXT("%s: AttachmentMesh Does NOT have the ComponentTag set in the project settings added, Set this"), *Actor->GetName());
		UKismetSystemLibrary::PrintString(Actor->GetWorld(), Message, true, false, FLinearColor::Blue, 10.0f);
...
FSKGAttachmentParent USKGAttachmentFunctionLibrary::CreateAttachmentParentStruct(AActor* AttachmentParent, FString& Error)
{
...
FString USKGAttachmentFunctionLibrary::SerializeAttachmentParent(AActor* AttachmentParent, FString& Error)
{
	...
	if (AttachmentParent)
	{
		const FSKGAttachmentParent AttachmentStruct = CreateAttachmentParentStruct(AttachmentParent, Error);
...
AActor* USKGAttachmentFunctionLibrary::ConstructAttachmentParent(UObject* WorldContextObject, FSKGAttachmentParent AttachmentStruct, AActor* Owner, APawn* Instigator, FString& Error)
{
	...
	{
		if (AActor* AttachmentParent = World->SpawnActorDeferred<AActor>(AttachmentStruct.AttachmentClass, FTransform(), Owner, Instigator, ESpawnActorCollisionHandlingMethod::AdjustIfPossibleButAlwaysSpawn))
		{
			ISKGAttachmentInterface::Execute_SetLoadDefaultAttachments(AttachmentParent, false);
			...
			if (AttachmentManager)
			{
				...
				{
					TArray<USKGAttachmentComponent*> AttachmentComponentsComponents = AttachmentManager->GetAllAttachmentComponents(true);
...
bool USKGAttachmentFunctionLibrary::SaveStringToFile(const FString& Path, const FString& FileName,
	const FString& FileContent)
{
	return FFileHelper::SaveStringToFile(FileContent, *(Path + FileName));
}

bool USKGAttachmentFunctionLibrary::LoadFileToString(const FString& Path, const FString& FileName, FString& OutString)
{
	return FFileHelper::LoadFileToString(OutString, *(Path + FileName));
}

bool USKGAttachmentFunctionLibrary::GetAllFiles(FString Path, TArray<FString>& OutFiles)
{
