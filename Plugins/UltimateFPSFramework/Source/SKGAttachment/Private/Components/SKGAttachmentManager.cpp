...
void USKGAttachmentManager::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_WITH_PARAMS_FAST(USKGAttachmentManager, AttachmentComponents, Params);
}
...
void USKGAttachmentManager::HideAllAttachments(bool bHide)
{
	TArray<AActor*> ActorsToDestroy;
	ActorsToDestroy.Reserve(CachedAttachmentComponents.Num());
	for (int i = 0; i < CachedAttachmentComponents.Num(); ++i)
	{
		...
		if (AttachmentComponent)
		{
			...
			if (IsValid(Attachment))
			{
				Attachment->SetHidden(bHide);
			}
		}
	}
}

void USKGAttachmentManager::HideAll(bool bHide)
{
	HideAllAttachments(bHide);
	if (GetOwner())
	{
		GetOwner()->SetHidden(bHide);
	}
}

void USKGAttachmentManager::SetCollisionAllAttachments(bool bEnable)
{
	TArray<AActor*> ActorsToDestroy;
	ActorsToDestroy.Reserve(CachedAttachmentComponents.Num());
	for (int i = 0; i < CachedAttachmentComponents.Num(); ++i)
	{
		...
		if (AttachmentComponent)
		{
			...
			if (IsValid(Attachment))
			{
				Attachment->SetActorEnableCollision(bEnable);
			}
		}
	}
}

void USKGAttachmentManager::SetCollisionAll(bool bEnable)
{
	SetCollisionAllAttachments(bEnable);
	if (GetOwner())
	{
		GetOwner()->SetActorEnableCollision(bEnable);
	}
}
...
void USKGAttachmentManager::AddAttachment(USKGAttachmentComponent* AttachmentComponent)
{
	if (IsValid(AttachmentComponent))
	{
		...
		MARK_PROPERTY_DIRTY_FROM_NAME(USKGAttachmentManager, AttachmentComponents, this);
