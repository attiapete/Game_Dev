...
#include "Misc/SKGAttachmentDeveloperSettings.h"
...
USKGAttachmentComponent::USKGAttachmentComponent()
{
	...
	CustomizationOffset = FVector::ZeroVector;
}
...
void USKGAttachmentComponent::HandleAttachmentConstruction()
{
	if (!DefaultAttachment.IsNull())
	{
		...
		if (GetAttachmentManager())
		{
			...
			{
				bSpawnDefaultParts = ISKGAttachmentInterface::Execute_GetShouldLoadDefaultAttachments(OwningActor);
			}
		}
		...
		if (bSpawnDefaultParts)
		{
			...
			if (bLoadAsyncByDefault)
			{
				...
				StreamableManager.RequestAsyncLoad({DefaultAttachment.ToSoftObjectPath()}, Delegate, FStreamableManager::DefaultAsyncLoadPriority, false, false);
				return;
			}
...
void USKGAttachmentComponent::PostNetReceive()
{
	...
	if (PreviewStatic || PreviewSkeletal)
	{
		PreviewSkeletal->DestroyComponent();
		PreviewStatic->DestroyComponent();
		PreviewSkeletal = nullptr;
		PreviewStatic = nullptr;
	}
}
...
void USKGAttachmentComponent::OnRep_Attachment()
{
	if (IsValid(Attachment) && Attachment->GetClass()->ImplementsInterface(USKGAttachmentInterface::StaticClass()))
	{
		...
		Attachment->SetActorRelativeTransform(FTransform());
	}
	if (GetAttachmentManager())
	{
...
void USKGAttachmentComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyName.Equals("bRefreshPreviewPosition"))
	{
		bRefreshPreviewPosition = false;
		SetPreviewPositions();
		return;
	}
			...
			if (const UBlueprintGeneratedClass* BPClass = Cast<UBlueprintGeneratedClass>(SelectedDataAsset->Attachments[SelectedDisplayIndex].ActorClass.LoadSynchronous()))
			{
				...
				for (const USCS_Node* Node : ActorBlueprintNodes)
				{
					if (Node && Node->ComponentTemplate)
					{
						...
						{
							if (const USKGAttachmentDeveloperSettings* Settings = GetDefault<USKGAttachmentDeveloperSettings>())
							{
								if (Node->ComponentTemplate->ComponentHasTag(Settings->AttachmentMeshTag))
								{
...
FVector USKGAttachmentComponent::GetCustomizationOffsetLocation() const
{
	if (GetOwner())
	{
		return UKismetMathLibrary::TransformLocation(GetOwner()->GetActorTransform(), GetRelativeLocation() + CustomizationOffset);
	}
...
bool USKGAttachmentComponent::Server_ClearCurrentAttachment_Validate()
{
	return true;
}

void USKGAttachmentComponent::Server_ClearCurrentAttachment_Implementation()
{
	ClearCurrentAttachment();
}

void USKGAttachmentComponent::ClearCurrentAttachment()
{
	if (!HasAuthority())
	{
		if (bAllowClientAuthorativeAttachmentChange)
		{
			Server_ClearCurrentAttachment();
		}
	}
	else
	{
...
AActor* USKGAttachmentComponent::AddExistingAttachment(AActor* INAttachment, bool bDestroyCurrentAttachment)
{
	...
	else
	{
		...
		if (IsValid(INAttachment))
		{
			INAttachment->ForceNetUpdate();
		}
	}
	return bDestroyCurrentAttachment ? Attachment : CurrentAttachment;
}
...
void USKGAttachmentComponent::AddNewAttachment(AActor* INAttachment, bool bDestroyCurrentAttachment)
{
	...
	{
		INAttachment->SetOwner(GetOwner());
...
void USKGAttachmentComponent::SetPreviewAttachment(TSubclassOf<AActor> AttachmentClass)
{
	if (IsValid(PreviewAttachment))
	{
		if (PreviewAttachment->Implements<USKGAttachmentInterface>())
		{
			TArray<USKGAttachmentComponent*> Components = ISKGAttachmentInterface::Execute_GetAllAttachmentComponents(PreviewAttachment, true);
			for (USKGAttachmentComponent* Component : Components)
			{
				if (Component && Component->GetAttachment())
				{
					Component->GetAttachment()->Destroy();
				}
			}
		}
	...
	if (AttachmentClass)
	{
		PreviewAttachment = GetWorld()->SpawnActorDeferred<AActor>(AttachmentClass, FTransform(), GetOwner());
		
		if (PreviewAttachment)
		{
			PreviewAttachment->SetReplicates(false);
			const FTransform SpawnTransform = GetComponentTransform();
			UGameplayStatics::FinishSpawningActor(PreviewAttachment, SpawnTransform);
			...
			PreviewAttachment->AttachToComponent(this, TransformRules);
		}
	}

	if (Attachment && !Attachment->IsHidden())
	{
		TArray<USKGAttachmentComponent*> AttachmentsComponents = ISKGAttachmentInterface::Execute_GetAllAttachmentComponents(Attachment, true);
		Attachment->SetActorHiddenInGame(true);
		for (USKGAttachmentComponent* Component : AttachmentsComponents)
		{
			if (Component && Component->GetAttachment())
			{
				Component->GetAttachment()->SetActorHiddenInGame(true);
			}
		}
	}
}

void USKGAttachmentComponent::ClearPreviewAttachment()
{
	if (IsValid(PreviewAttachment))
	{
		if (PreviewAttachment->Implements<USKGAttachmentInterface>())
	...
	if (Attachment && Attachment->IsHidden())
	{
		...
		for (USKGAttachmentComponent* Component : AttachmentsComponents)
		{
			if (Component && Component->GetAttachment())
			{
				Component->GetAttachment()->SetActorHiddenInGame(false);
			}
		}
	}
}