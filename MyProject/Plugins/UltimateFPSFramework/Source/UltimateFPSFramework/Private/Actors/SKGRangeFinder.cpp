...
void ASKGRangeFinder::SetupAttachmentMesh()
{
	...
	if (const USKGAttachmentDeveloperSettings* Settings = GetDefault<USKGAttachmentDeveloperSettings>())
	{
		...
		{
			if (IsValid(TextComponent) && TextComponent->ComponentHasTag(Settings->AttachmentMeshTag))
			{
	...
	{
		UE_LOG(LogTemp, Error, TEXT("%s: AttachmentMesh Does NOT have the ComponentTag set in the project settings added, Set this"), *GetName());
	}
}
