...
{
	GENERATED_BODY()
	UPROPERTY(BlueprintReadWrite, Category = "SKGFPSFramework|Attachments")
	...
	TArray<AActor*> GetAllActors()
	{
		...
		if (Barrel && Barrel->GetAttachment())
			Actors.Add(Barrel->GetAttachment());
		if (Handguard && Handguard->GetAttachment())
			Actors.Add(Handguard->GetAttachment());
		if (Stock && Stock->GetAttachment())
			Actors.Add(Stock->GetAttachment());
		if (ForwardGrip && ForwardGrip->GetAttachment())
			Actors.Add(ForwardGrip->GetAttachment());
		if (Muzzle && Muzzle->GetAttachment())
			Actors.Add(Muzzle->GetAttachment());
		if (Magazine && Magazine->GetAttachment())
			Actors.Add(Magazine->GetAttachment());
		if (Bipod && Bipod->GetAttachment())
			Actors.Add(Bipod->GetAttachment());
		
		for (const USKGAttachmentComponent* Component : Parts)
			if (Component && Component->GetAttachment())
				Actors.Add(Component->GetAttachment());
		for (const USKGAttachmentComponent* Component : Sights)
			if (Component && Component->GetAttachment())
				Actors.Add(Component->GetAttachment());
		for (const USKGAttachmentComponent* Component : LightLasers)
			if (Component && Component->GetAttachment())
				Actors.Add(Component->GetAttachment());
		for (const USKGAttachmentComponent* Component : Magnifiers)
			if (Component && Component->GetAttachment())
				Actors.Add(Component->GetAttachment());
		for (const USKGAttachmentComponent* Component : RenderTargets)
			if (Component && Component->GetAttachment())
				Actors.Add(Component->GetAttachment());
...
struct FSKGAimCameraSettings
{
	...
	// How close/far should the camera be from this optic when aiming
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "SKGFPSFramework")
	float ThirdPersonCameraDistance = 0;
	// This will lock the distance from optic to the camera when aiming. Useful for magnified optics with a simulated eyebox
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "SKGFPSFramework")
	bool bThirdPersonUseFixedCameraDistance = false;
};
...
struct FSKGFirearmStats
{
	...
	static inline void UpdateStats(FSKGFirearmStats& FirearmStats, const FSKGFirearmStats& DefaultFirearmStats, const FSKGFirearmPartStats& PartStats)
	{
		...
		//FirearmStats.VerticalRecoilMultiplier = DefaultFirearmStats.VerticalRecoilMultiplier;
		FirearmStats.VerticalRecoilMultiplier *= PercentMultiplier;
...
struct FSKGInfraredMaterialSettings
{
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "SKGFPSFramework", meta = (EditCondition = "bUseInfraredMaterialSettings", EditConditionHides))
	...
	TObjectPtr<UMaterialParameterCollectionInstance> InfraredMPC = nullptr;
...
{
	GENERATED_BODY()
	UPROPERTY(EditDefaultsOnly, NotReplicated, BlueprintReadWrite, Category = "SKGFPSFramework")
